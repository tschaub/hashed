...
Store.prototype._debouncedUpdate = function(values) {
  var existingValues = this._values;
  for (var i = 0, ii = this._providers.length; i < ii; ++i) {
    ...
    schema.forEachKey(function(key, prefixed) {
      ...
      if (!(prefixed in values)) {
        var deserializedDefault = schema.getDefault(key);
        if (existingValues[prefixed] !== schema.serialize(key, deserializedDefault)) {
          changed = true;
          state[key] = deserializedDefault;
        }
      } else if (existingValues[prefixed] !== values[prefixed]) {
        try {
          deserialized = schema.deserialize(key, values[prefixed]);
        } catch (err) {
          return;
        }
